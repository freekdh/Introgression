...
// [[Rcpp::depends(BH)]]
/// work on probability of extinction in the output.
/// make sure each Rcpp function can only run sequentially. Initialize - Run - WriteOutput.
/// To allow multithreading don't push back on the DataSet vector... Initialize the vector of size N and have each realization of the for loop write to a specific element. 
...
bool INITIALIZESIMULATION_FUN = false, RUNSIMULATION_FUN = false;
...
void CollectParameters(double &r, int &nloci, int &nploidy, int &ninit0, int &ninit1, int &distlocal, double &scmajor, double &sclocal, int &ngen, double &rec, int &k){    
    //General 
