...
Parameters::Parameters(const Rcpp::List &parslist){
    MUTATIONRATE = 0.0;
    INTRINSIC_GROWTHRATE = parlist["r"];
    NLOCI = parlist["nloci"];
    NPLOIDY = parlist["nploidy"];
    NINIT[0] = parlist["ninit0"];
    NINIT[1] = parlist["ninit1"];
    NLOCAL_ADAPTED_LOCI = 1;
    DISTLOCAL = parlist["distlocal"];
    SC_MAJOR = parlist["scmajor"];
    SC_LOCAL = parlist["sclocal"];
    NGEN = parlist["ngen"];
    NREP = parlist["nrep"];
    RECOMBINATIONRATE = parlist["rec"];
    K = parlist["k"];
...
Rcpp::List Rcpp_IntrogressionSimulation(Rcpp::List parslist, int setthreads = 0, bool progressbar = true){
    ...
    #endif
    if(progressbar == true) Progress p(nrep, true);
